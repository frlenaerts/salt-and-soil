...
- Each log line is tagged with a scope (`None` = global, or a set of source aliases); `/api/state` and `/api/stream` filter the log, mounts, and error banner per user so nobody sees activity for sources they can't access. The SSE delta is computed on the full log (global position) and then projected to the lines a user may see.
- Only one scan/sync runs at a time on the shared runtime. `try_begin_scan`/`try_begin_sync` claim it synchronously (no `await` between check and state flip → atomic, no lock); a conflicting request gets `409` naming the busy user. The scheduler claims the same way and yields to manual runs.
